...
chrome.action.onClicked.addListener(async (activeTab) => {
  try {
    const jwts = await chrome.cookies
      .getAll({ url: activeTab.url })
      .then((cookies) =>
        cookies
          .filter(
            (cookie) =>
              cookie.name.startsWith("jwt") ||
              cookie.name.startsWith("_jwt") ||
              cookie.name.startsWith("_identityJwt")
          )
          .map((cookie) => {
